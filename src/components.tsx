...
export function ProgressBar({
  value, color, trackColor, label, showPct = true,
}: {
  value: number; color: string; trackColor: string; label: string; showPct?: boolean;
}) {
...
export function StatCard({
  ...
  return (
    <view
      ...
      gap={4}
    >
      <text fontSize={10} fontWeight={700} color={C.textMuted}>{label.toUpperCase()}</text>
      <text fontSize={26} fontWeight={800} color={color}>{value}</text>
      <text fontSize={11} color={C.textMuted}>{sub}</text>
    </view>
  );
}
