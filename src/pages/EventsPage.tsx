...
export function EventsPage() {
  const [eventLog, setEventLog] = useState<Array<{ type: string; ts: number; n: number }>>([]);
  ...
  return (
    <view display="flex" flexDir="col" gap={0} h="full" scrollable>
      ...
      <view display="flex" flexDir="col" gap={24} p={24}>
        ...
        <view
          onClick={() => { setClicks(c => c + 1); push('onClick'); }}
          onMouseDown={() => { setDowns(d => d + 1); push('onMouseDown'); }}
          onMouseUp={() => { setUps(u => u + 1); push('onMouseUp'); }}
          w="full"
          h={110}
          bg={C.surface2}
          hover:bg={C.surface3}
        ...
        >
          <view display="flex" flexDir="col" items="center" gap={4}>
            <text fontSize={18} fontWeight={700} color={C.textSub} hover:color={C.text}>
              Click / Press Here
            </text>
            <text fontSize={12} color={C.textMuted}>
        ...
        <view display="flex" flexDir="col" gap={10}>
          <text fontSize={14} fontWeight={700} color={C.text}>hover: / active: prop variants</text>
        ...
        <view display='flex' flexDir='col' p={20} bg={C.surface} rounded={12} border={1} borderColor={C.border}>
          <view display="flex" flexDir="row" items="center" justify="between" mb={12}>
            <view display="flex" flexDir="row" items="center" gap={8}>
              ...
              <Badge label={`${seq} total`} color={C.accentHi} bg={C.accentDark} />
            </view>
            <button
          ...
          <view scrollable h={220} display='flex' flexDir='col' >
            {eventLog.length === 0 ? (
              <view p={20} display="flex" items="center" justify="center">
                <text fontSize={13} color={C.textMuted}>Interact with the hit target above to see events here.</text>
              </view>
            ) : eventLog.map((e, i) => (
              <view
                key={e.n}
